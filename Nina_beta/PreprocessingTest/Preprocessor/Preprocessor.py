...
# 	def _lemmatise_xml(self, input_file):
# 		lem_call = "Preprocessor/CST_tools/cstlemma -X -Xwword -Xppos -Xllemma -Xsw -L -l -f Preprocessor/CST_tools/flexrules -i " + input_file
# 		lem_dict = subprocess.check_output(lem_call, shell=True, stderr= subprocess.STDOUT)
# 		print lem_dict
...
# 	def make_poslem_corpus(self, dir_path):
# 		if self.test: dir_path = "data/postagger_output"
# 		all_files = glob.glob(dir_path + "/*.in")
...
# 				for row in articles[article][1:]:
# 					tmp = []
# 					x = [x for x in [y.split("/") for y in row.split(" ")]]
# 					for y in x:
# 						if len(y)==2:
# 							tmp.append([y[0], y[1]])
# 					monster_article.append(tmp)

# 				for i in range(1, len(articles[article])):
# 					articles[article][i] = articles[article][i].replace("/", " /")

# 				self._write_to_file(articles[article])
# #				self._write_to_xmlfile(monster_article)
# #				temp = self._lemmatise_xml(self.input_file + ".xml")
# 				temp = self._lemmatise_xml(self.input_file)
# 				print temp
# 				articles[article] = self._split_up_output(date, temp)
...
# 				pickle.dump(articles, file)
	...
	# def _write_to_xmlfile(self, input_content_list):
	# 	with open(self.input_file + ".xml", "w") as fi:
	# 		fi.write("<?xml version=\"1.0\" encoding=\"utf-8\"?><article>")
	# 		for row in input_content_list:
	# 			string = "<element>"
	# 			for x in row:
	# 				string = string + "<w><lemma></lemma><word>" + x[0] + "</word>"
	# 				string = string + "<pos>" + x[1] + "</pos></w>"
